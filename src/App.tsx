...
function Dashboard({ onLogout }: DashboardProps) {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      {/* Header - Top Bar */}
      <header className="h-[64px] bg-white border-b border-slate-200 px-6 flex items-center justify-between flex-shrink-0 z-50">
        <div className="flex items-center gap-8">
           <button 
             onClick={() => setCustomer(null)}
             className="text-[10px] font-black text-slate-500 hover:text-primary transition-colors uppercase tracking-[0.2em] border-r border-slate-200 pr-8 py-2 cursor-pointer"
           >
             Workspace
           </button>
           ...
           <div className="md:flex flex-1 min-w-[300px] relative">
            <div className="relative w-full">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2 bg-slate-100 border border-slate-200 rounded-lg text-sm outline-none focus:border-primary/30 transition-all font-medium"
              />
            </div>
            ...
            <AnimatePresence>
              {isSearchFocused && filteredCustomers.length > 0 && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 5 }}
                  className="absolute top-full mt-2 left-0 right-0 bg-white border border-slate-200 shadow-xl overflow-hidden z-[100]"
                >
                  <div className="p-3 text-[10px] font-black text-slate-500 uppercase tracking-widest bg-slate-50 border-b border-slate-200">
                    Results
                  </div>
                  <div className="max-h-[400px] overflow-y-auto">
                    {filteredCustomers.map((customer) => (
                      <button
                        ...
                        className="w-full text-left p-4 hover:bg-primary-light/30 border-b border-slate-200 last:border-0 flex items-center gap-4 transition-colors"
                      >
                        <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-black text-base">
                          {customer.companyName.charAt(0)}
                        </div>
                        <div>
                          <div className="text-[14px] font-black text-slate-900 leading-tight">{customer.companyName}</div>
                          <div className="text-[11px] text-slate-500 font-bold">{customer.id}</div>
                        </div>
                      </button>
                    ))}
                  </div>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex flex-col items-end mr-2">
            <div className="text-[14px] font-black text-slate-900 leading-tight">Sarah Mitchell</div>
            <div className="text-[11px] text-slate-500 font-bold uppercase tracking-tighter">Corporate Admin • Finance</div>
          </div>
          ...
          <button 
            ...
            className="md:flex hidden w-10 h-10 items-center justify-center text-slate-500 hover:text-primary transition-colors bg-slate-50 border border-slate-200 rounded-lg ml-2"
          >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-[260px] bg-white border-r border-slate-200 flex flex-col flex-shrink-0">
            ...
            {menuItems.map((item) => (
              ...
              >
                <item.icon size={20} className={cn(
                  "transition-colors",
                  activeTab === item.id ? "text-primary" : "text-slate-500 group-hover:text-slate-900"
                )} />
          ...
          <div className="p-4 border-t border-slate-200 mt-auto space-y-3">
            <button 
              onClick={onLogout}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[14px] font-black text-slate-500 hover:text-red-600 hover:bg-red-50 transition-all"
            >
              <Trash2 size={20} />
              <span>Logout</span>
            </button>
        ...
        <main className={cn(
          "flex-1 grid bg-slate-200 gap-[1px] overflow-hidden transition-all duration-300",
          isRightPanelOpen ? "grid-cols-1 xl:grid-cols-[1fr_320px]" : "grid-cols-1"
        )}>
          
          {/* Main Workspace Area */}
          <section className="bg-slate-50 overflow-y-auto p-4 sm:p-6 relative scroll-smooth thin-scrollbar">
            <div className={cn("mx-auto mb-10 transition-all", isRightPanelOpen ? "w-full" : "max-w-6xl")}>
              <div className="flex flex-col mb-8 mt-2">
                 ...
                 <div className="flex items-center justify-between gap-4">
                   ...
                   <button className="bg-white border border-slate-200 text-[11px] font-black px-4 py-2 rounded-xl text-slate-900 hover:bg-slate-50 transition-colors uppercase tracking-widest flex items-center gap-2">
                     <History size={14} /> Last updated 2 min ago
                   </button>
                 </div>
                 <p className="text-[13px] text-slate-500 font-bold mt-2 uppercase tracking-wide">
                    ...
                    <div className="space-y-6">
                      <div className="bg-white p-6 rounded-xl border border-slate-200">
                        <div className="flex items-center justify-between mb-8">
                          <div>
                            <h2 className="text-[18px] font-black text-slate-900 uppercase tracking-tight">Active Master Service Agreements</h2>
                            <p className="text-[12px] text-slate-500 font-medium">Governing terms and contractual artifacts</p>
                          </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {[
                            ...
                            <div key={idx} className="p-4 rounded-xl bg-slate-50 border border-slate-200 hover:border-primary/30 transition-all cursor-pointer group">
                              ...
                              <h4 className="text-[14px] font-black text-slate-900 group-hover:text-primary transition-colors">{doc.title}</h4>
                              <div className="flex items-center justify-between mt-4">
                                <span className="text-[10px] font-bold text-slate-500 uppercase font-mono">{doc.ref}</span>
                                <span className="text-[10px] font-bold text-slate-500 uppercase">Exp: {doc.expiry}</span>
                              </div>
                            </div>
                          ))}
                        </div>
